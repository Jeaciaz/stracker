  ...
  "SpendingRepo",
  {
    ...
    effect: Effect.gen(function* () {
      ...
      const del = SqlSchema.single({
        Result: Schema.Struct({ id: SpendingId }),
        Request: SpendingId,
        execute: (request) => sql`
      ...
      const clean = SqlSchema.void({
        Request: Schema.Void,
        execute: () => sql`delete from spendings`,
      });
        ...
        update: (payload: SpendingUpdatePayload) =>
          update(payload).pipe(
            Effect.catchTag("NoSuchElementException", () =>
              Effect.fail(new SpendingNotFoundError({ id: payload.id })),
            ),
            Effect.catchTags({
              SqlError: Effect.die,
